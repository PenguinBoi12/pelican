...
def test_discover_migration_files__with_non_py_files__expect_ignored(
    tmp_path: Path,
) -> None:
    (tmp_path / "1_create_users.py").write_text("")
...
def test_discover_migration_files__with_missing_directory__expect_error(
    tmp_path: Path,
) -> None:
...
def test_load_migration_file__expect_migration_registered(
...
def test_load_migrations__expect_all_files_registered(
    tmp_path: Path, registry: MigrationRegistry
) -> None:
...
def test_load_migrations__expect_registry_cleared_before_load(
    tmp_path: Path, registry: MigrationRegistry
) -> None:
