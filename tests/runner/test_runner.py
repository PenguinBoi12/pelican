...
def _index_names(runner: MigrationRunner, table: str) -> list[str]:
    return [i["name"] for i in inspect(runner.engine).get_indexes(table)]


def test_upgrade__expect_version_recorded(db_runner):
...
def test_downgrade__expect_version_removed(db_runner):
    migration = Migration(name="init", revision=1)
    migration.up = lambda: None
...
def test_upgrade__with_multiple_migrations__expect_all_versions_tracked(db_runner):
    for rev in [1, 2, 3]:
...
def test_upgrade__with_no_up_function__expect_error(db_runner):
    migration = Migration(name="init", revision=1)
    with pytest.raises(ValueError, match="no upgrade function"):
        db_runner.upgrade(migration)


def test_downgrade__with_no_down_function__expect_error(db_runner):
    migration = Migration(name="init", revision=1)
    with pytest.raises(ValueError, match="no downgrade function"):
        db_runner.downgrade(migration)


def test_create_table__expect_table_exists(db_runner):
...
def test_create_table__expect_columns_created(db_runner):
    with create_table("animals") as t:
        t.string("name")
        t.integer("age")
...
def test_create_table__with_timestamps__expect_timestamp_columns(db_runner):
...
def test_create_table__with_index__expect_index_exists(db_runner):
...
def test_create_table__with_unique_index__expect_unique_flag(db_runner):
...
def test_change_table__with_new_column__expect_column_added(db_runner):
...
def test_change_table__with_rename__expect_column_renamed(db_runner):
...
def test_change_table__with_drop__expect_column_removed(db_runner):
...
def test_change_table__with_alter_on_sqlite__expect_not_implemented(db_runner):
...
def test_change_table__with_alter_on_new_table__expect_error(db_runner):
    with pytest.raises(ValueError, match="alter can only be used on existing table"):
        with create_table("new_table") as t:
            t.alter("col")


def test_drop_table__expect_table_removed(db_runner):
